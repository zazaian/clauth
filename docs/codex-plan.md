...
- **A live session does not hot-swap accounts** (`AuthManager::auth`, `manager.rs:2023-2034`; the guarded reload's identity check, `:2109-2124`): every request reads `auth_cached()`, the in-memory value behind `inner: RwLock<CachedAuth>` — disk is touched only when `should_refresh_proactively` fires on THAT cached token's own JWT `exp`. That reload then checks the on-disk `account_id` against the one it expects; a relink (or any different-account login) changes it, so `reload_if_account_id_matches` returns `Skipped` and `refresh_token` turns that into a permanent error carrying `REFRESH_TOKEN_ACCOUNT_MISMATCH_MESSAGE` ("...you have since logged out or signed in to another account. Please sign in again.") rather than adopting the new account — `auth()` catches the error and keeps serving the stale cached token. A running `codex`/`codex exec` process therefore never observes a clauth switch (TUI or CLI) until it exits and a new one starts, whose first `auth()` call has nothing cached yet and loads the freshly-relinked file straight from disk. No external trigger forces it either: `AuthManager` never registers `auth.json` with the `file-watcher` crate (present in-tree, unused by `login/src/auth`) and owns no signal handler; the only unconditional (unguarded) `.reload()` call sites are the app-server's own `login_*`/`logout_*` RPC handlers (`app-server/src/request_processors/account_processor.rs`) and the `codex login` CLI's own short-lived process (`cli/src/main.rs:1833`), and both drive a real login inside that same process rather than accepting an outside nudge to re-read the file.
